...
SELECT nombre 
FROM producto WHERE precio>=60 AND precio<=120;
...
SELECT nombre,precio 
FROM producto WHERE precio>=180 ORDER BY nombre ASC , precio DESC; 
...
FROM producto JOIN fabricante ON fabricante.id=producto.id_fabricante;
...
FROM fabricante JOIN producto  ON fabricante.id=producto.id_fabricante GROUP BY fabricante.id;
...
FROM fabricante JOIN producto ON producto.id_fabricante=fabricante.id WHERE producto.precio>=(SELECT AVG(producto.precio) FROM producto) GROUP BY fabricante.id;	
-- 15 Nombre y precio del producto mas barato.
SELECT nombre,precio 
FROM producto WHERE precio = (SELECT MIN(precio) FROM producto);
...
FROM fabricante JOIN producto ON precio = (SELECT MAX(precio) FROM producto);
...
UPDATE producto SET nombre = 'Impresora Laser' WHERE nombre = 'printer' ;
